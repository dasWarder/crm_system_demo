...
public class TaskServiceIntegrationTest extends AbstractTest {
  ...
  public void shouldUpdateTaskByIdProperly() throws TaskNotFoundException {
    ...
    Task updatedTask =
        taskService.updateTaskById(TestTaskData.TEST_TASK_2.getId(), TestTaskData.TEST_UPDATE_TASK);
  ...
  public void shouldGetTasksProperly() {
    ...
    List<Task> allTasksList =
        Stream.of(
                TestTaskData.TEST_TASK_1, TestTaskData.TEST_TASK_2,
  ...
  public void shouldGetTasksFroDateProperly() {
    ...
    List<Task> expectedTasks =
        Stream.of(TestTaskData.TEST_TASK_3, TestTaskData.TEST_TASK_4).collect(Collectors.toList());
  ...
  public void shouldGetMissedTasksProperly() {
    ...
    List<Task> expectedTasks =
        Stream.of(TestTaskData.TEST_TASK_1, TestTaskData.TEST_TASK_2, TestTaskData.TEST_TASK_3)
            .collect(Collectors.toList());
    List<Task> actualTasks = taskService.getMissedDeadlineTasks(TEST_PAGEABLE).getContent();
  ...
  public void shouldGetActualTasksBetweenProperly() {
    ...
    List<Task> actualTasks =
        taskService.getActiveTasksBetween(start, end, TEST_PAGEABLE).getContent();
    Assertions.assertNotNull(actualTasks);
    assertThat(actualTasks)
        .usingRecursiveComparison()
        .ignoringFields("todoList")
  ...
  public void shouldGetTasksByParamProperly() throws UnsupportedParameterException {
    ...
    String query = "Create";
    List<Task> expectedTasks =
        Stream.of(TestTaskData.TEST_TASK_1, TestTaskData.TEST_TASK_2, TestTaskData.TEST_TASK_4)
            .collect(Collectors.toList());
    List<Task> actualTasks =
        taskService.getTasksByParams(filters, query, TEST_PAGEABLE).getContent();
    Assertions.assertNotNull(actualTasks);
    assertThat(actualTasks)
        .usingRecursiveComparison()
        .ignoringFields("todoList")
