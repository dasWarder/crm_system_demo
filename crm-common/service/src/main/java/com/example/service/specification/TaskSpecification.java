...
public class TaskSpecification {
  ...
  public Specification<Task> findTasksByParams(final String[] filters, final String query)
    ...
    Specification<Task> taskSpecification =
        (r, cq, cb) -> {
          ...
          Predicate resultPredicate = cb.or(predicates.toArray(new Predicate[predicates.size()]));
  ...
  private List<String> findUnsupportedParams(String[] filters) {
    ...
    List<String> unsupportedParams =
        Arrays.stream(filters)
            .filter(f -> !f.equalsIgnoreCase("title") && !f.equalsIgnoreCase("description"))
